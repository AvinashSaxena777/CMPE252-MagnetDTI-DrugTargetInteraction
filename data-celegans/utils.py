...
def multiomics_data():
    ...
    ESM = np.genfromtxt("data-celegans/02protein_ESM.csv", delimiter=',', skip_header=1, dtype=np.dtype(str))
    protein = ESM[:, 0]
    protein_number = len(protein)
    ESM = np.array(ESM)
    ...
    # drug_fea
    PC = np.genfromtxt("data-celegans/02drug_phy_chem.csv", delimiter=',',skip_header=1,dtype=np.dtype(str))
    drug = PC[:,0]
    ...
    MACCS = np.genfromtxt("data-celegans/02drug_MACCS.csv", delimiter=',',skip_header=1,dtype=np.dtype(str))
    drug = MACCS[:, 0]
    drug_number = len(drug)
    ...
    #加载label
    labellist = []
    with open('data-celegans/03label_unique.txt', 'r') as file:
